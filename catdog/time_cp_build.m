...
for i=1:NUMTRIALS
    ...
    %tic
    %tStart = cputime;
    [tnsList,walltime,cpu_time] = doctns(N,words,wordToIndex,newFileNames,'format',"htensor");
    htns_elapsed = htns_elapsed + walltime;
    htns_cpu = htns_cpu + cpu_time;

    tStart = cputime;
    for j=1:N
        %decompose each document tensor
        f = @() htns_cp_als2(tnsList{j},50);
...
for i=1:NUMTRIALS
    ...
    %tic
    %tStart = cputime;
    [tnsList,walltime,cpu_time] = doctns(N,words,wordToIndex,newFileNames,'format',"sptensor");
